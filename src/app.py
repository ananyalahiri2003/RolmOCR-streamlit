...
CACHE_DIR = os.getenv("CACHE_DIR", os.path.join(tempfile.gettempdir(), "rolmocr_cache"))
...
def load_model(_device):
    ...
    try:
        logger.info(f"Loading RolmOCR model on {_device}")
        processor = AutoProcessor.from_pretrained("reducto/RolmOCR", cache_dir=CACHE_DIR)
...
def process_single_image(image, prompt_text="What is shown in this image?", device="cpu", show_progress_bar=None, max_new_tokens=1500):
...
def process_batch(images, prompt_text, device, progress_bar=None, max_new_tokens=1500):
...
def display_history():
    ...
    for item in reversed(st.session_state.history):
        with st.expander(f"#{item['id']} - {item['type']} ({item['timestamp']})"):
            ...
            with tabs[0]:
                st.text_area("Output", item['result'])
                st.download_button(
                    "Download Text",
                    item['result'],
...
def main():
    ...
    # App header
    st.markdown('<p class="main-header">Rolm OCR App</p>', unsafe_allow_html=True)
    st.markdown('<p class="sub-header">Analyze text from images using RolmOCR</p>', unsafe_allow_html=True)

    # Check dependencies
    missing_deps = check_dependencies()
    if missing_deps:
        st.error(f"Missing dependencies: {', '.join(missing_deps)}. Please install them to use this app.")
        st.info("Install with: poetry update " + " ".join(missing_deps))
        st.stop()
    ...
    with st.sidebar:
        ...
        selected_device = st.selectbox(
            ...
            available_devices,
            index=0 if len(available_devices) == 0 else 1, # Default to cuda if available
        ...
        if device.startswith("cuda"):
            with st.expander("CPU Memory Management"):
        ...
        # About section
        with st.expander("About Rolm OCR"):
            st.write("""
            This app uses RolmOCR, a powerful OCR model for document understanding from Hugging Face Hub.

            The app extracts document info and returns plain text for easy reading.

            Available tasks:
            - Extract info from this page.
            
            You could also enter a custom_prompt if you are interested in using this app.
            """)
        ...
        try:
            ...
            if len(images) > 0:
                with st.spinner(f"Processing {len(images)} images..."):
                    ...
                    results = process_batch(images, final_prompt, device, progress_bar, max_new_tokens=max_new_tokens)
                    ...
                    for idx, (result, image) in enumerate(zip(results, images)):
                        with st.expander(f"Image {idx + 1} Results"):
                            ...
                            with col2:
                                ...
                                with inner_tabs[0]:
                                    st.text_area("Plain Text", result, height=200)
                                    st.download_button(
                                        f"Download Text",
                                        result,
